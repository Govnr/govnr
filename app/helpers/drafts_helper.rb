module DraftsHelper
  ...
  def drafts_form(config = {}, &block)
    form_for(@draft, { as: :draft, url: url_for(controller: :drafts, action: :update), html: { class: 'wiki_form', method: :post } }.merge!(config), &block)
  end

  def draft_diff(old_text, new_text)
    puts "DRAFT_DIFF"
    p @draft.id
    @compare = Comparators::Compare.comparator
    p old_text
    p new_text
    @compare.render_changes(h(old_text), h(new_text)).html_safe

  end

  # def comparator
  #   comparator = Comparators::DiffLcs.new
  # end
